...
> **首次编译须知**：macOS 的辅助功能权限绑定到应用签名。从源码编译默认使用 ad-hoc 签名（每次编译身份不同），导致每次重新编译都需要重新去「系统设置 → 隐私与安全性 → 辅助功能」里打勾授权。解决方法是创建一次自签名证书（30 秒）：
> ```
> Keychain Access → 证书助理 → 创建证书
> 名称: Pastry Dev  |  身份类型: 自签名根  |  证书类型: 代码签名
> ```
> 之后 `./deploy.sh` 会自动使用该证书签名，辅助功能权限永久保留。Release 同理，证书名改为 `Pastry Release`。
>
