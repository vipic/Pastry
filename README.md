...
- **链接预览** — 复制 URL 自动抓取 og:title / og:description / og:image，无 og:image 时降级用页面首张有效图片，卡片内缩略图 + 标题 + 描述混排
...
- **LinkPreviewLoader** — og:title / og:description / og:image 多格式解析，无 og:image 时降级扫描页面首张有效图片（跳过 data URI / 1×1 像素）
